...
public class KSPSolarSystem {
	...
	private void addStockBodies() {
		...
		KSPCelestialBody eve = new KSPCelestialBody(5,  "EVE",    "Eve",      sun,    700000, 8.171730229E12, 85109365.0,  eveOrbit, 3.14, Color.MAGENTA);
		bodies.add(eve);
		Orbit gilOrbit = new Orbit(12.0,   0.55,          3.15E7,   80.0,  10.0);
		KSPCelestialBody gil = new KSPCelestialBody(13, "GILLY",  "Gilly",    eve,     13000, 8.2894498E6,      126123.27, gilOrbit, 0.9,  Color.GRAY);
		bodies.add(gil);
		
		Orbit kerOrbit = new Orbit( 0.0,   0.0,   1.3599840256E10,   0.0,   0.0);
		KSPCelestialBody ker = new KSPCelestialBody(1,  "KERBIN", "Kerbin",   sun,    600000,    3.5316E12,   84159286.0,  kerOrbit, 3.14, Color.BLUE);
		bodies.add(ker);
		Orbit munOrbit = new Orbit( 0.0,   0.0,            1.2E7,    0.0,   0.0);
		KSPCelestialBody mun = new KSPCelestialBody(2,  "MUN",    "The Mun",  ker,    200000, 6.5138398E10,    2429559.1,  munOrbit, 1.7,  Color.GRAY);
		bodies.add(mun);
		Orbit minOrbit = new Orbit( 6.0,   0.0,            4.7E7,   78.0,  38.0);
		KSPCelestialBody min = new KSPCelestialBody(3,  "MINMUS", "Minmus",   ker,     60000,    1.7658E9,     2247428.4,  minOrbit, 0.9,  Color.GREEN);
		bodies.add(min);
		
		Orbit dunOrbit = new Orbit( 0.06,  0.051, 2.0726155264E10, 135.5,   0.0);
		KSPCelestialBody dun = new KSPCelestialBody(6,  "DUNA",   "Duna",     sun,    320000, 3.01363212E11,  47921949.0,  dunOrbit, 3.14, Color.RED);
		bodies.add(dun);
		Orbit ikeOrbit = new Orbit( 0.2,   0.03,           3.2E6,    0.0,   0.0);
		KSPCelestialBody ike = new KSPCelestialBody(7,  "IKE",    "Ike",      dun,    130000, 1.8568369E10,    1049598.9,  ikeOrbit, 0.7,  Color.GRAY);
		bodies.add(ike);
		
		Orbit dreOrbit = new Orbit( 5.0,   0.145, 4.0839348203E10, 280.0,  90.0);
		KSPCelestialBody dre = new KSPCelestialBody(15, "DRES",   "Dres",     sun,    138000, 2.1484489E10,   32832840.0,  dreOrbit, 3.14, Color.GRAY);
		bodies.add(dre);
		
		Orbit jooOrbit = new Orbit( 1.304, 0.05,  6.8773560320E10,  52.0,   0.0);
		KSPCelestialBody joo = new KSPCelestialBody(8,  "JOOL",   "Jool",     sun,   6000000, 2.82528004E14, 2455985200.0, jooOrbit, 0.1,  Color.GREEN);
		bodies.add(joo);
		Orbit layOrbit = new Orbit( 0.0,   0.0,         2.7184E7,    0.0,   0.0);
		KSPCelestialBody lay = new KSPCelestialBody(9,  "LAYTHE", "Laythe",   joo,    500000,     1.962E12,    3723645.8,  layOrbit, 3.14, Color.BLUE);
		bodies.add(lay);
		Orbit valOrbit = new Orbit( 0.0,   0.0,         4.3152E7,    0.0,   0.0);
		KSPCelestialBody val = new KSPCelestialBody(10, "VALL",   "Vall",     joo,    300000,   2.07482E11,    2406401.4,  valOrbit, 0.9,  Color.CYAN);
		bodies.add(val);
		Orbit tylOrbit = new Orbit( 0.025, 0.0,           6.85E7,    0.0,   0.0);
		KSPCelestialBody tyl = new KSPCelestialBody(12, "TYLO",   "Tylo",     joo,    600000,   2.82528E12,   10856518.0,  tylOrbit, 3.14, Color.LIGHT_GRAY);
		bodies.add(tyl);
		Orbit bopOrbit = new Orbit(15.0,   0.235,        1.285E8,   10.0,  25.0);
		KSPCelestialBody bop = new KSPCelestialBody(11, "BOP",    "Bop",      joo,     65000,  2.4868349E9,    1221060.9,  bopOrbit, 0.9,  Color.GRAY);
		bodies.add(bop);
		Orbit polOrbit = new Orbit( 4.25,  0.171,       1.7989E8,    2.0,  15.0);
		KSPCelestialBody pol = new KSPCelestialBody(14, "POL",    "Pol",      joo,     44000,  7.2170208E8,    1042138.9,  polOrbit, 0.9,  BROWN);
		bodies.add(pol);
	...
	public ArrayList<KSPCelestialBody> getBodiesOrbiting(String cName) {
		int cid = -1;
		KSPCelestialBody c = getBodyByName(cName);
		
		if (c != null) {
			cid = c.getID();
		}
		
		return getBodiesOrbiting(cid);
	}
	
	public ArrayList<KSPCelestialBody> getBodiesOrbiting(int cid) {
		ArrayList<KSPCelestialBody> returnList = new ArrayList<KSPCelestialBody>();
		Iterator<KSPCelestialBody> it = bodies.iterator();
		while (it.hasNext()) {
			KSPCelestialBody bod = it.next();
    		if (bod.hasParent() && bod.getParent().getID() == cid) {
    			returnList.add(bod);
    		}
		}
		return returnList;
