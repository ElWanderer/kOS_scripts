...
GLOBAL SAT_NAME IS "Lander Test 4".
GLOBAL CORE_HEIGHT IS 2. // metres

GLOBAL LAND_BODY IS MINMUS.
GLOBAL LAND_LAT IS 0.
GLOBAL LAND_LNG IS 180.
GLOBAL SAFETY_ALT IS 2000.

GLOBAL PARK_ORBIT IS 30000.
GLOBAL PARK_I IS MAX(87.5,MIN(90,ABS(LAND_LAT)+5)). // try to ensure we fly-over target latitude
GLOBAL RETURN_ORBIT IS 20000.
...
IF rm < 0 {
  ...
  store("doLanderAscent("+RETURN_ORBIT+",90,0,851).").
  doLanderAscent(RETURN_ORBIT,90,0,851).
